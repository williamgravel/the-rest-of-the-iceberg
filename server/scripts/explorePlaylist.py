...
connLibrary = sqlite3.connect('../databases/UserLibraryDB.sqlite')
...
conn = sqlite3.connect('../databases/ExplorePlaylistDB_' + userSpotifyID + '.sqlite')
cur = conn.cursor()
...
for artist in topArtists:
    ...
    cur.execute('INSERT OR IGNORE INTO TopArtists (spotify_id, name) VALUES (?, ?)', (artistID, artistName))
...
for topArtistID in topArtistsList:
    secondURL = 'https://api.spotify.com/v1/artists/' + topArtistID + '/related-artists'
    ...
    for artist in relatedArtists:
        ...
        artistID = artist['id']
        artistName = artist['name']
        cur.execute('INSERT OR IGNORE INTO RelatedArtists (spotify_id, name, top_artist_id) VALUES (?, ?, ?)', (artistID, artistName, topArtistID))

        conn.commit()

    cur.execute('SELECT RelatedArtists.spotify_id FROM RelatedArtists WHERE RelatedArtists.top_artist_id = ? ORDER BY RANDOM() LIMIT ?', (topArtistID, relatedArtistsLimit))
...
for relatedArtistID in relatedArtistsMasterList:
    ...
    req = requests.get(thirdURL, params=thirdQueryParameters, headers=headerFields)
    jsonData = req.json()
    ...
    for track in topTracks:
        ...
        cur.execute('INSERT OR IGNORE INTO TopTracks (spotify_id, spotify_uri, name, related_artist_id) VALUES (?, ?, ?, ?)', (trackID, trackURI, trackName, relatedArtistID))

    conn.commit()

    cur.execute('SELECT TopTracks.spotify_uri FROM TopTracks WHERE TopTracks.related_artist_id = ? ORDER BY RANDOM() LIMIT ?', (relatedArtistID, topTracksLimit))
...
data = '{"name":"Explore Your Taste","description":"This playlist was generated by analyzing your favorite artists within the past ' + timeRangeMsg + ' and choosing their related artists top songs.","public":false}'

#res = requests.post(fourthURL, headers=headerFields, data=playlistOptions)
res = requests.post(fourthURL, headers=headerFields, data='{"name":"Explore Your Taste"}')
jsonData = res.json()
...
for loop in range(totalLoops):
    playlistTracks = json.dumps({'uris': topTracksMasterList[loop*100:(loop+1)*100]})
