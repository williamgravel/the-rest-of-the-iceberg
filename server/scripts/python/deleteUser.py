...
cur.execute(
    'DELETE FROM UserTrackList WHERE UserTrackList.user_id = ?', (userDatabaseID,))
...
cur.execute(
    'DELETE FROM Albums WHERE NOT EXISTS (SELECT * FROM Tracks WHERE Tracks.album_id = Albums.id)')

# FIFTH STEP - DELETE ARTISTS LINKED WITH DELETED ALBUMS IN ARTISTS TABLE
cur.execute(
    'DELETE FROM Artists WHERE NOT EXISTS (SELECT * FROM Albums WHERE Albums.artist_id = Artists.id)')
